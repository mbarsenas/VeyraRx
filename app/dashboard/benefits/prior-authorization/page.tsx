...
export default async function PriorAuthorizationPage({ searchParams }: { searchParams: Promise<{ scenario?: string }> }) {
  ...
  return (
    <>
      <MemberTopbar eyebrow="Benefits & coverage" title="Prior authorization" description="Track medication requests that require clinical review before plan coverage can be determined." />
      {scenarioId && <PbmScenarioContext scenario={scenario} />}

      <article className="panelCard">
        <div className="panelHeader">
          <div><span className="eyebrow">Utilization management</span><h2>Current authorization activity</h2><p className="railText">Prior authorization is a coverage review. It does not guarantee payment, a specific member cost or dispensing until the pharmacy claim is adjudicated.</p></div>
        ...
        {priorAuthorizations.length > 0 ? priorAuthorizations.map((item) => (
          <article className="paItem" key={`${item.medication}-${item.lastUpdated}`}>
            <div className="panelHeader"><div><span className="eyebrow">Medication request</span><h2>{item.medication}</h2></div><span className={item.status.toLowerCase().includes("approved") ? "statusChip processing" : "statusChip attention"}>{item.status}</span></div>
      ...
      <article className="panelCard" style={{ marginTop: "20px" }}>
        <span className="eyebrow">Member guidance</span><h2>What usually happens next</h2>
